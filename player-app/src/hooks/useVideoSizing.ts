...
export const useVideoSizing = (
  ...
  const updateVideoSize = useCallback(() => {
    ...
    if (window.innerWidth > window.innerHeight * (9 / 16)) {
      ...
      const uiContainer: HTMLElement | null = wrapperRef.current.querySelector(
      ...
      wrapperRef.current.setAttribute(
        "data-video-width",
        Math.round(viewportWidth).toString()
      );
